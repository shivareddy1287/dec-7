...
const OrgAddProfile = () => {
  ...
  // console.log("🚀 ~ file: OrgAddProfile.js:88 ~ OrgAddProfile ~ users:", users);
  const { isProfileAdded, appErr, serverErr, profilesList, managersList } =
    users;
  console.log(managersList?.[0]?.id, "managersList");
  ...
  const formik = useFormik({
    ...
    initialValues: {
      ...
      managerDataId: managersList?.[0]?.id,
    ...
    onSubmit: (values) => {
      ...
      console.log(values.email, "ProjectTeam values");
    },
  });

  if (
    isProfileAdded ||
    (!proAdminAccessGivenFun(getAccessData?.Access) && getAccessData?.Access)
  )
