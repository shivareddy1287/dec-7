...
const OrgUpdateProfile = () => {
  ...
  const { appErr, serverErr, isProfileUpdated, profilesList, managersList } =
    users;
  const getAccessData = profilesList?.find(
    (item) => item?._id === users?.userAuth?._id
  );
  // console.log(getAccessData?.Access, "getaccess");
  // if (isProfileUpdated || !normalAdminAccessGivenFun(getAccessData?.Access))
  ...
  if (
    ...
    return <Navigate to={`/organization/profile`} />;
  console.log(managerDataId?._id, "managerDataId", managersList);
  return (
    <div>
